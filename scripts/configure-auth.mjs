...
`,{mode:0o600});
function cli(command){const r=spawnSync('npx',['--yes','supabase@2.117.0','config',command,'--yes','--project-ref',expected.projectRef,'--workdir',wd],{encoding:'utf8',timeout:90000,maxBuffer:4e6});if(r.status!==0)throw Error('Auth configuration failed: '+r.stderr.slice(-500));return r.stdout;}
...
if(process.argv[2]==='apply'){cli('push');console.log('Auth origins/templates applied to',expected.purpose,expected.projectRef)}else console.log('Auth diff prepared at',wd+'/review.diff');

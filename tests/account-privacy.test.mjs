...
test('production policy restricts scripts, connections, framing and browser sensors without allowing JS eval',()=>{
 ...
 assert.match(policy,/connect-src 'self' blob: https:\/\/project.supabase.co https:\/\/project.storage.supabase.co;/);assert.ok(policy.includes("frame-ancestors 'none'"));
 assert.ok(!policy.includes('*.supabase.co'),'TUS upload permits its exact Storage origin, not a wildcard');

...
try {
 for(const [name,engine] of [['chromium',chromium],['webkit',webkit]]){
  ...
  if(errors.length)throw Error(errors.join('\n'));summary.push({browser:name,editToggles:20,singleEditor:true,draftOnFocus:true,failedSavePreservesDraft:true,discardDialog:true,oneWrite:true,tabKeyboard:true,viewports:[320,390,430,768],realHeicDecode:true,wireFormat:mediaFixture.wireFormat,crop:meta.width+'x'+meta.height,privateMetadataStripped:true,accountSwitch:true});await browser.close();
 }

...
- Texto brasileiro, direto e curto. Nome de produto: Pico.
- Aparência escura, grafite e areia; superfícies translúcidas usadas com intenção.
...
- Reutilize Button, Input, GlassPanel, Avatar e SportChip antes de duplicar.
...
- createClient do navegador retorna null sem configuração e a interface trata esse estado.
...
- Testar políticas como dois usuários diferentes; contas de teste são explicitamente identificadas.
...
- npm run typecheck
- npm run build
- npm run dev e smoke HTTP das rotas alteradas.
- Validar fluxos reais quando o backend estiver configurado; informar quando não foi possível.
- Testes devem cobrir comportamento e riscos, não repetir a implementação.

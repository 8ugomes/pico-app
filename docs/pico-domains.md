...
| Entrada e autenticação | [Contrato oficial](OFFICIAL_COMMUNITY_AUTH.md), [ambientes](ENVIRONMENTS.md), `AuthForm`, `AuthPage`, `AccessGate`, `SessionGuard`, `src/lib/auth` | Cadastro por senha sem confirmação nesta beta, admissão vigente e recuperação por e-mail explicitamente indisponível; suspensão/revogação/exclusão não restauradas pelo redesign. |
| Configuração e assistência | [ONBOARDING.md](ONBOARDING.md), [contrato oficial](OFFICIAL_COMMUNITY_AUTH.md), `GuidedOnboarding`, `OfficialWelcome`, `src/lib/onboarding.ts`, `connected/ProfileEditor` | Perfil com foto, nome, usuário e modalidade antes de explorar; Conta e edição própria acessíveis; guia opcional e aviso institucional separados; preferências isoladas por conta; progresso do guia não é sucesso social. |
...
| Pessoas e perfis | [Perfil](CYCLE10_PROFILE.md), [busca](SEARCH.md), [contratos](CYCLE9_CONTRACTS.md), `DiscoverView`, `PlayerCard`, `connected/ConnectedDiscovery`, `connected/ConnectedProfile` | Acompanhar é unilateral; histórico privado não alimenta descoberta; nenhum e-mail alheio no perfil. |
...
| Notificações | [Contrato](NOTIFICATIONS.md), [menções](COMMUNITY_MENTIONS.md), `NotificationsProvider`, `NotificationsView`, `NotificationLink`, `/api/notifications` | Avisos por destinatário após entrada ativa ou menção em publicação; participação atual, bloqueio, admissão e acesso ao post; leitura explícita, demo vazio e nenhuma notificação do sistema. |

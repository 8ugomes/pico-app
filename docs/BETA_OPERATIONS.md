...
Quatro buckets privados: `avatars`, `post-media`, `entity-media`, `post-videos`. Servidor verifica identidade e autorização vigente antes de usar Storage administrativo. Imagens passam novamente por normalização sem metadados; resposta sem cache/URL assinada de leitura. Vídeos MP4 até 45 MiB (47.185.920 bytes) usam token TUS assinado para um caminho em `/resumable/sign`; finalização confere tamanho, MIME e cabeçalho. O limite global atual do Supabase Free é 50 MB por arquivo; confirmar o plano e o limite global antes de elevar novamente o bucket. Cada trecho reproduzido exige autorização atual e não entrega a URL assinada ao navegador. A conta lista as reservas de vídeo não publicadas para remoção, limitadas a oito simultâneas, sem contar vídeos já publicados. Remoção em duas fases bloqueia anexação concorrente. O token TUS não é revogado instantaneamente pela remoção: um token retido pode recriar um objeto órfão privado até expirar. Acompanhar Storage sem linha correspondente em `post_video_assets` e remover esses objetos pela API após o prazo do token; nunca apagar linhas de `storage.objects` diretamente. Arquivo de recurso continua pertencendo à arena/grupo após saída do uploader.
...
`database.mjs migrate` e `deploy.mjs` capturam inventários privados de posts, comentários, jogos, perfis, contexto de jogos, referências de mídia, arenas, participações, comunidades e vínculos comunidade/arena antes/depois. Recibos em `.vercel/content-preservation/` contêm identificadores e hashes, sem texto/foto/senha. Perda de linha ou mudança de autoria/data interrompe o processo; novas contribuições e edições são contabilizadas. Investigar diferenças, inclusive exclusões legítimas concorrentes, antes de continuar. Nunca restaurar automaticamente nem copiar conteúdo pessoal para Git. A conferência não substitui backup cifrado, verificação da mídia nem teste de visibilidade: uma linha pode continuar salva e ser ocultada por RLS. [Caso real, correção e regressões](social-refinement-review/README.md).

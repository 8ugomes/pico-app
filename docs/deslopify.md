...
Na publicação, o stage e o domínio principal responderam a versão `bda869824b42`, saúde OK e acesso anônimo protegido; a CSP continuou incluindo o host exato de Storage. Não houve sessão pessoal nem postagem artificial em produção. A amostra visual e o upload autenticado >30 MiB seguem sendo evidência do desenvolvimento, não de aparelhos reais.
...
A integração manteve menções inline, navegação ao post, busca global e fotos existentes. Na amostra servida, o avatar da pessoa permanece visível no cartão de notificação a 320 px, inclusive com texto a 200%; o fallback humano funciona sem foto. O cabeçalho estreito ficou numa linha, com busca por ícone acessível. O compositor deixa foto/vídeo mutuamente exclusivos e trava a escolha durante o envio; a opção de vídeo ficou lado a lado no rodapé estreito após substituir o `fieldset` que quebrava os rádios em duas linhas. A arena em 320 px mostra acompanhar, registrar jogo privado e declaração pública em controles distintos, com consequência escrita; o perfil apresenta a arena sob “Já joguei”, separado de “Arenas acompanhadas”. Amostras 320/390/1280 px em claro/escuro não tiveram overflow horizontal; telas de carregamento/erro e texto ampliado preservaram ação recuperável. Um MP4 real gerado no Chrome foi decodificado/reproduzido no teste hospedado, sem exame em aparelho físico. Arenas sem confirmação não ganharam endereço, esporte ou foto inventados.
...
Produção: o mesmo build validado foi promovido ao endereço principal após CI. Versão e saúde pública conferidas; não houve inspeção autenticada do compositor com dados reais em produção. A amostra visual conectada acima permanece a evidência da interação.
## Composição direta — critérios antes de codar · 14/09/2026

A ação Publicar deve levar ao texto com foco imediato, sem uma pilha inicial de caixas. Foto é gesto de ferramenta; audiência e destinos são escolhas legíveis, compactas e recuperáveis antes do envio. A lista de pessoas nasce junto ao `@` e desaparece quando a expressão sai do texto; mostrar nome e @usuário sem pedir uma busca num segundo campo. `@todos` deve ser uma sugestão, não checkbox. Indicar a comunidade escolhida quando houver mais de uma. Não esconder o público real nem notificar alguém por texto que pareça menção sem seleção. No registro privado, arena/modalidade/data são necessárias, mas o gesto e a hierarquia devem parecer um diário rápido; manter a privacidade explícita. Conferir foco, teclado, 320px, claro/escuro e rascunho em falha.

Revisão local: a caixa de texto assume o primeiro foco; foto, contador e público ficam em uma linha de decisão, com ajustes disponíveis sem preencher uma ficha. A inspeção de 320px revelou o seletor de arquivo e rótulos visíveis no fixture de CSS, além de título/cancelamento comprimidos no diário. A ocultação acessível foi explicitada nos estilos do componente, o diário ganhou largura livre e o vazio não compete com o registro aberto. O navegador conectado revelou que o input oculto ainda estendia o documento até 426px; sua posição foi ancorada ao controle, e 390/320/1280px passaram depois. A busca de menção é contextual à comunidade, e `@nome` sem seleção recebe orientação em vez de aviso implícito. Build, amostra claro/escuro e CI de navegador Chromium/WebKit aprovados; versão pública confirmada.

## Refino do compositor e arena de destino — critérios antes de codar · 14/09/2026

O editor aberto ainda parece uma ficha: texto, ferramentas, configurações e ações ocupam faixas separadas. Reunir escrita e envio em uma composição calma, com foto acessível e público legível antes da ação principal. As opções avançadas aparecem sob demanda, sem repetir instruções quando a consequência já está clara. “Mural de arena” precisa de campo de busca no lugar de uma lista extensa sem ordem; mostrar nomes ordenados, resultado vazio e arena escolhida de forma distinta. Busca não muda o destino salvo sem clique explícito. Revisar largura estreita, texto ampliado, foco por teclado, claro/escuro e estado após falha de envio.

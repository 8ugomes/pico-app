...
105 testes locais na branch isolada e 108 após integrar a main `055d2cf` passaram, junto de lint, typecheck, build conectado e auditoria npm sem vulnerabilidades. O smoke com 15 verificações foi repetido no app compilado após resolver a integração. Supabase de desenvolvimento: 61 verificações reais em Auth, API, RPC, Storage e navegador; duas contas, um grupo privado e suas fotos removidos. O teste confirmou busca de perfil real, acesso por link, normalização, limites/anonimato, bloqueio e manutenção da privacidade ao abrir a comunidade encontrada. [Resultado hospedado](hosted.json). Demo: cinco verificações sem backend/escritas.

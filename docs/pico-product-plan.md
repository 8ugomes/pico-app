## Arenas, identidade nas notificações e vídeo nos posts · IMPLEMENTADO LOCALMENTE, PUBLICAÇÃO PENDENTE · 14/09/2026
...
Gate hospedado no desenvolvimento: 247 verificações reais de Auth, banco, Storage e HTTP, incluindo TUS assinado em `/resumable/sign`, faixas privadas, revogação, declaração sem post, avatar do autor e decodificação/reprodução de um MP4 real gerado no Chrome; fixtures removidas. Navegador verificou notificações a 320 px e texto ampliado, arena/perfil conectados em 320/390/1280 px, conta com marca oculta e compositor/menções nos dois temas. O cabeçalho estreito voltou a uma linha com busca acessível por ícone. Lint, typecheck, build, 160 testes locais e audit sem vulnerabilidade alta passaram. Isso não comprova reprodução em aparelho físico nem validação de todos os codecs/perfis MP4. Um backup cifrado novo do principal, com 11 objetos, hash e permissões, precede a migration. Restam PR/CI, preflight final do catálogo e ledger principal, stage/promoção e smoke de produção.

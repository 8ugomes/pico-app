...
- Inventário de preservação ampliado a arenas, participações, comunidades, vínculos e mídia de recursos. Importação idempotente conserva IDs, slugs, conteúdo e alterações da gestão.
- Lint/typecheck/build, 142 testes, regressões finais e smoke real de busca/arquivos em desenvolvimento passaram. [Pesquisa, evidências e limites](arena-regional-review/README.md).
...
- 132 testes locais, lint/typecheck/build e 50 verificações hospedadas aprovados. [Pesquisa, operação e limites](arena-state-research/validacao.md). Publicação autorizada via PR/CI/main e script oficial.
...
- Inventários privados antes/depois de migrations e deployments detectam perda/substituição de conteúdo; novos posts e edições são registrados, sem restauração automática. 118 testes locais, 104 verificações hospedadas, lint/types/build e revisão visual aprovados. [Evidências e estado da entrega](social-refinement-review/README.md).
...
- [PR #39](https://github.com/8ugomes/pico-app/pull/39) integrado com CI verde. Migration única no principal preservou 124/124 registros; bucket privado e restrição ficaram em 45 MiB. Stage `dpl_5UhvvdzsJPTBKNK38paD6ZBfKcHN` promovido ao `pico-app-sepia.vercel.app`, versão pública `bda869824b42`, saúde e proteções anônimas verificadas. Nenhum plano pago contratado nem conteúdo de teste criado no principal. [Recibo e limites](pico-product-plan.md#recibo-da-publicação).

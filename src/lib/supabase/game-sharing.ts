...
export function parseGameShare(body: Record<string, unknown>) {
  exactKeys(body, ['id', 'version', 'key', 'body', 'imagePath', 'videoPath', 'audience', 'wallArena', 'groups']);
  ...
  if (body.videoPath !== null && body.videoPath !== undefined && typeof body.videoPath !== 'string') return invalid();
  if (body.imagePath && body.videoPath) return invalid();
  return { id: uuid(body.id), version: Number(body.version), key: uuid(body.key), body: textField(body.body, 0, 500),
    imagePath: body.imagePath as string | null | undefined, videoPath: body.videoPath as string | null | undefined, audience: body.audience as 'beta' | 'private',

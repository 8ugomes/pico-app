...
export function GamesView({ initialArenaId }: { initialArenaId?: string }) {
  ...
  return <>
    ...
    {!open && <Button data-tour="register-game" onClick={() => {if(editing){setEditing(null);setArenaId('');setSportId('');setDate('');}setError('');setOpen(true);setMessage('');}}>Registrar jogo</Button>}
    {message && <p role="status" className="inline-success">{message}</p>}
    {(!open || games.length > 0) && <GameEntries games={games} shareControl={game => <PostComposer key={`${game.id}:${game.version}`} game={game} />} onEdit={g => {setEditing(g);setArenaId(g.arena_id);setSportId(g.sport_slug);setDate(g.played_on);setError('');setOpen(true);}} onDelete={setDeleting} />}
    {open && <section className="game-capture" aria-label={editing ? 'Corrigir jogo' : 'Registrar jogo'}><div className="game-capture-heading"><div><span>Meu diário · demonstração</span><h2>{editing ? 'Corrigir este jogo' : 'Joguei aqui'}</h2></div><Button variant="quiet" size="small" onClick={() => setOpen(false)}>Cancelar</Button></div><form className="connected-form" onSubmit={e => {
      e.preventDefault();
    ...
    }}><label className="input-group">Onde você jogou?<select className="input" required value={arenaId} onChange={e => {setArenaId(e.target.value);setSportId(state.arenas.find(a => a.id === e.target.value)?.sports[0] ?? '');}}><option value="">Escolha a arena</option>{state.arenas.map(a => <option value={a.id} key={a.id}>{a.name} · Demo</option>)}</select></label>
    {arena && <label className="input-group">Modalidade<select className="input" required value={sportId} onChange={e => setSportId(e.target.value as SportId)}>{arena.sports.map(s => <option key={s} value={s}>{state.sports.find(x => x.id === s)?.name}</option>)}</select></label>}
    <GameDateField value={date} onChange={setDate} />{error && <p role="alert" className="form-error">{error}</p>}<Button type="submit" disabled={!sportId || !date}>{editing ? 'Salvar correção' : 'Guardar só para mim'}</Button></form></section>}

...
export function ProfilePlaces({ playerId }: { playerId: string }) {
  ...
  const played = useEntity<Place[]>(`/api/activity?kind=played&player=${playerId}`);
  return <section className="profile-places"><h2>Vínculos no Pico</h2><p>{data?.own ? 'Arenas que você acompanha e comunidades de que participa.' : 'Lugares e grupos disponíveis para você consultar.'}</p>{error && <><p role="alert">{error}</p><Button variant="secondary" onClick={reload}>Tentar novamente</Button></>}{!data && !error && <p role="status">Carregando vínculos…</p>}{data && <><h3>Arenas acompanhadas</h3><div className="context-links">{data.arenas.map(a => <Link href={`/arenas/${a.slug}`} key={a.id}>{a.name}</Link>)}</div>{!data.arenas.length && <p>Nenhuma arena acompanhada disponível. {data.own && <Link className="journey-text-link" href="/arenas">Explorar arenas</Link>}</p>}<h3>Já joguei</h3><p>{data.own ? 'Arenas que você escolheu declarar publicamente. Seus jogos e datas continuam privados.' : 'Arenas em que esta pessoa declarou ter jogado.'}</p>{played.error && <p role="alert">{played.error} <Button size="small" variant="quiet" onClick={played.reload}>Tentar novamente</Button></p>}{played.loading && <p role="status">Carregando declarações…</p>}{played.data && <><div className="context-links">{played.data.map(a => <Link href={`/arenas/${a.slug}`} key={a.id}>{a.name}</Link>)}</div>{!played.data.length && <p>Nenhuma arena declarada.</p>}</>}<h3>Comunidades</h3><div className="context-links">{data.communities.map(c => <Link key={c.id} href={`/comunidades/${c.slug}`}>{c.name}</Link>)}</div>{!data.communities.length && <p>Nenhuma comunidade disponível para sua conta. {data.own && <Link className="journey-text-link" href="/comunidades">Conhecer comunidades</Link>}</p>}</>}</section>;
}

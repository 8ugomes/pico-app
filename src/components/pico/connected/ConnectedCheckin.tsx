...
export function ConnectedCheckin({ initialSlug }: { initialSlug?: string }) {
  ...
  return <>
    ...
    {data && <>
      ...
      {presence.map(p => <article className="connected-panel" key={p.id}><strong>{p.name}</strong><p><MapPin size={14} aria-hidden="true" /> <Link href={`/arenas/${p.arena.slug}`}>{p.arena.name}</Link> · {p.sport.name}</p></article>)}

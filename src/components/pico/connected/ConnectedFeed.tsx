...
function EditPost({post,onDone}:{post:PostRecord;onDone:()=>void}){const[open,setOpen]=useState(false),[body,setBody]=useState(post.body),[busy,setBusy]=useState(false),[message,setMessage]=useState('');return <><Button size="small" variant="quiet" onClick={()=>setOpen(true)}>Editar publicação</Button><Modal open={open} onClose={()=>{if(!busy)setOpen(false)}} title="Editar publicação"><form onSubmit={async e=>{e.preventDefault();setBusy(true);try{await entityAction('/api/posts',{action:'edit',id:post.id,body});setOpen(false);onDone()}catch(e){setMessage(e instanceof Error?e.message:'Não foi possível salvar.')}finally{setBusy(false)}}}><label className="input-group">Texto<textarea className="input" maxLength={500} required value={body} onChange={e=>setBody(e.target.value)}/></label><Button disabled={busy}>Salvar alteração</Button><p role="status">{message}</p></form></Modal></>}

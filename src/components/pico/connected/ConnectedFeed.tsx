...
function ConnectedPost({ post, refresh }: { post: FeedRow; refresh: () => void }) {
  ...
  return <article className="post-card">
    <header className="post-header"><Link href={`/perfil/${post.username}`} className="post-person"><span className="read-profile-avatar"><UserRound size={24} aria-hidden="true" /></span><span><strong>{post.display_name}</strong><small><time dateTime={post.created_at}>{new Date(post.created_at).toLocaleString('pt-BR', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}</time></small></span></Link><span className="sport-label"><SportIcon sport={post.sport_slug} />{post.sport_name}</span></header>

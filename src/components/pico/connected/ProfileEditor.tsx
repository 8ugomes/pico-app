...
import { ArrowLeft, Check, UserRound, Volleyball } from 'lucide-react';
...
export function ProfileEditor({ profile, done, onAvatarChange = () => {}, setup }: {
  ...
  return <div className="profile-v2 profile-editor-v2" data-testid="profile-editor">
    <header className="profile-v2-edit-heading">
      ...
      <div><h1 ref={heading} tabIndex={-1}>{initialSetup ? 'Seu lugar no Pico' : 'Editar perfil'}</h1><p>{initialSetup ? 'Sua foto, seu nome e seu esporte.' : 'Mostre quem você é dentro e fora da areia.'}</p></div>
    </header>
    {initialSetup && <nav className="profile-setup-progress" aria-label="Configuração do perfil">
      ...
      <a href={`#${prefix}-photo`}>{profile.avatarPath ? '✓ ' : ''}Sua foto</a><a href={`#${prefix}-about`}>Nome e usuário</a><a href={`#${prefix}-sport`}>Seu esporte</a>
    </nav>}
    <section id={`${prefix}-photo`} className="profile-editor-v2-photo" aria-label="Foto do perfil">
      ...
      <div className="profile-editor-v2-photo-control"><h2>Sua foto</h2><p>{profile.avatarPath ? 'Foto salva.' : 'Escolha uma foto para a turma reconhecer você.'}</p>
    ...
    {state.status === 'success' && state.data.kind === 'sports' && <form className="profile-editor-v2-form" onSubmit={submit} aria-label="Editar informações do perfil">
      <fieldset disabled={blocked} className="profile-editor-v2-fields">
        <section className="profile-editor-v2-section" aria-labelledby={`${prefix}-about`}>
          ...
          <Input id={`${prefix}-username`} name="username" label="Nome de usuário" value={draft.username} onChange={e => field('username', e.target.value.toLowerCase())} minLength={3} maxLength={40} pattern="[a-z0-9_]{3,40}" autoCapitalize="none" spellCheck={false} hint="Use letras, números ou _." required />
        </section>
        ...
        <details className="profile-editor-v2-section profile-personalization" open={!initialSetup}>
          <summary id={`${prefix}-location`}>Personalizar perfil <span className="input-hint">· opcional</span></summary>
        ...
        {!profile.onboardingCompleted && <p className="form-note">Ao salvar, seu perfil fica visível no Pico e você entra na comunidade oficial. Você pode sair do grupo quando quiser.</p>}
        <div className="profile-editor-v2-save">{!initialSetup && <p>{dirty ? 'Você tem alterações para salvar.' : 'Seu e-mail e sua senha ficam em Minha conta.'}</p>}<div><Button type="submit" disabled={!state.data.sports.length || blocked}><Check size={18} aria-hidden="true" />{mutation.busy ? 'Salvando…' : initialSetup ? 'Salvar perfil e entrar' : 'Salvar perfil'}</Button>{!initialSetup && <Button type="button" variant="quiet" disabled={blocked} onClick={cancel}>Cancelar edição</Button>}</div></div>

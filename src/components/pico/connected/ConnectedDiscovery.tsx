...
export function ConnectedDiscovery() {
  ...
  return <>
    ...
    {data && <><div className="list-heading"><span role="status">{players.length} {players.length === 1 ? 'jogador' : 'jogadores'} nesta página</span><Button size="small" variant="quiet" disabled={refreshing} onClick={refresh}>{refreshing ? 'Atualizando…' : 'Atualizar'}</Button></div>
      ...
      {!settled && !sport && !arena && !level && <CommonPlayers />}
      {!players.length && <EmptyState title="Nenhuma pessoa nesta busca.">Tente outro nome ou @usuário. Se houver filtros selecionados, experimente limpá-los.</EmptyState>}

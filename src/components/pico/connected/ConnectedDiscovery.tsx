...
export function ConnectedDiscovery() {
  ...
  return <>
    ...
    {options && <details className="discovery-filters"><summary>Filtrar pessoas{(sport || arena || level) ? ` · ${[sport,arena,level].filter(Boolean).length} filtro(s)` : ""}</summary><div className="connected-form"><fieldset>
      ...
      <label className="input-group">Arena acompanhada<select className="input" value={arena} onChange={e => { setArena(e.target.value); setOffset(0); }}><option value="">Todas as arenas</option>{options.arenas.map(a => <option value={a.id} key={a.id}>{a.name}{a.isDemo ? ' · Demo' : ''}</option>)}</select></label>

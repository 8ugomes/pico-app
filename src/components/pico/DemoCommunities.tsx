...
export function DemoCommunities({ arenaId }: { arenaId?: string }) {
  const { state } = useDemo();
  const [mine, setMine] = useState(false), [search, setSearch] = useState('');
  const groups = state.communities.filter(c => (arenaId ? c.arenaId === arenaId : !mine || c.members.includes(state.currentUserId) || c.pending.includes(state.currentUserId)) && normalizeSearch(c.name).includes(normalizeSearch(search.trim())));
  return <>
    {!arenaId && <><PageHeading title="Comunidades"><Link className="journey-text-link" href="/descobrir">Buscar pessoas</Link></PageHeading><p className="page-intro">Grupos ilustrativos para explorar como participar e continuar a conversa.</p><div className="feed-tabs"><button className={mine ? 'active-tab' : ''} aria-pressed={mine} onClick={() => setMine(true)}>Minhas comunidades</button><button data-tour="community-explore" className={!mine ? 'active-tab' : ''} aria-pressed={!mine} onClick={() => setMine(false)}>Explorar</button></div></>}
    <div className="community-toolbar"><SearchField maxLength={100} label={mine && !arenaId ? "Buscar nas minhas comunidades" : "Buscar comunidades"} placeholder="Nome da comunidade" value={search} onChange={setSearch} />{search && <Button variant="quiet" size="small" onClick={() => setSearch('')}>Limpar busca</Button>}</div>

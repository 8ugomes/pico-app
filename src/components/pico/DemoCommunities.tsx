...
export function DemoCommunities({ arenaId }: { arenaId?: string }) {
  ...
  return <>
    {!arenaId && <><PageHeading eyebrow="ENCONTRE QUEM JOGA COM VOCÊ" title="Comunidades" /><p className="page-intro">Grupos ilustrativos para explorar como participar e continuar a conversa.</p><div className="feed-tabs"><button className={mine ? 'active-tab' : ''} aria-pressed={mine} onClick={() => setMine(true)}>Minhas comunidades</button><button className={!mine ? 'active-tab' : ''} aria-pressed={!mine} onClick={() => setMine(false)}>Explorar</button></div></>}
    ...
    {!arenaId && <footer className="community-create"><div><h2>Quer criar uma comunidade real?</h2><p>É preciso uma conta com acesso aprovado. Esta demonstração não cria grupos no serviço.</p></div><Link className="journey-text-link" href="/signup">Criar minha conta <ArrowRight size={16} aria-hidden="true" /></Link></footer>}
  </>;
}
export function DemoCommunity({ slug }: { slug: string }) {
  ...
  return <><Link className="detail-back" href="/comunidades">← Comunidades</Link><header className="community-identity"><PageHeading eyebrow="COMUNIDADE · DEMONSTRAÇÃO" title={c.name} /><p className="community-description">{c.description}</p>{arena && <Link className="community-place" href={`/arenas/${arena.slug}`}>{arena.name}</Link>}<p className="community-conditions">{c.visibility === 'private' ? 'Conteúdo só para participantes ativos' : 'Conteúdo para pessoas aprovadas no beta'} · {c.entryMode === 'approval' ? 'entrada por aprovação' : 'entrada aberta'}</p></header>

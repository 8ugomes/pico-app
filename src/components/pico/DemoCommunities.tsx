...
export function DemoCommunities({ arenaId }: { arenaId?: string }) {
  ...
  return <>
    {!arenaId && <><PageHeading title="Comunidades" /><p className="page-intro">Grupos ilustrativos para explorar como participar e continuar a conversa.</p><div className="feed-tabs"><button className={mine ? 'active-tab' : ''} aria-pressed={mine} onClick={() => setMine(true)}>Minhas comunidades</button><button data-tour="community-explore" className={!mine ? 'active-tab' : ''} aria-pressed={!mine} onClick={() => setMine(false)}>Explorar</button></div></>}

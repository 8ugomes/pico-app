...
export function PostComposer({ fixedArenaId, fixedCommunityId, game, initiallyOpen = false }: { fixedArenaId?: string; fixedCommunityId?: string; game?: PlayedGame; initiallyOpen?: boolean }) {
  ...
  return <>
    ...
    <Modal open={open} onClose={() => setOpen(false)} title={game ? 'Compartilhar jogo' : 'Compartilhe com sua turma'}><form onSubmit={submit} className="connected-form">
      ...
      <label className="input-group">Audiência<select className="input" value={audience} onChange={e => { setAudience(e.target.value as 'beta' | 'private'); setGroups([]); setWall(false); }}><option value="beta">Pessoas aprovadas no beta · simulação</option><option value="private">Grupo privado · simulação</option></select></label>
      ...
      <p className="form-note">{audience === 'private' ? 'Só participantes ativos do grupo escolhido veriam este conteúdo.' : 'Esta audiência representa as pessoas aprovadas no beta.'} O post permanece apenas nesta sessão. Registrar um jogo não seleciona um mural nem publica automaticamente.</p>

...
import { ChevronDown, ImageIcon, PenLine, Share2, X } from 'lucide-react';
...
export function PostComposer({ fixedArenaId, fixedCommunityId, game, initiallyOpen = false }: { fixedArenaId?: string; fixedCommunityId?: string; game?: PlayedGame; initiallyOpen?: boolean }) {
  ...
  const editor = <form onSubmit={submit} className="connected-form compose-form">
    {!game && <div className="compose-heading"><span>Nova publicação · demonstração</span><button type="button" className="compose-close" aria-label="Fechar editor" onClick={() => setOpen(false)}><X size={19} aria-hidden="true" /></button></div>}
    ...
    <details className="compose-settings"><summary><span className="compose-settings-label">{audience === 'private' ? 'Só participantes do grupo' : 'Pessoas do Pico'}{destinations.length ? ` · ${destinations.join(' · ')}` : ' · seu perfil'}</span><span className="compose-settings-action">Alterar <ChevronDown size={16} aria-hidden="true" /></span></summary><div className="compose-settings-body">
    ...
    <div className="compose-footer"><div className="compose-footer-tools"><button type="button" className="compose-tool-label" aria-pressed={photo} onClick={() => setPhoto(value => !value)}><span className="compose-tool"><ImageIcon size={19} aria-hidden="true" />{photo ? 'Imagem ilustrativa adicionada' : 'Imagem ilustrativa'}</span></button><span className="compose-count">{content.length}/500</span></div><Button type="submit" disabled={(!content.trim() && !game) || (audience === 'private' && groups.length !== 1)}>Publicar nesta demonstração</Button></div>
  </form>;
  return <>

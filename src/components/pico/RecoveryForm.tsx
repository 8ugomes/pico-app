  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (reset) {
        ...
        const result=await client.auth.resetPasswordForEmail(String(data.get('email')).trim(),{redirectTo:new URL('/auth/confirm',location.origin).href});
        if (result.error) { setNotice({error:true,text:'Não foi possível enviar agora. Aguarde alguns minutos e tente novamente.'}); return; }
        setNotice({error:false,text:'Se houver uma conta para esse e-mail, você receberá um link. Ao abrir o link, confirme para continuar. Confira também o spam.'});
        form.reset();
      }

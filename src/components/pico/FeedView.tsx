...
export function FeedView() {
  ...
  const posts = visibleDemoPosts(state).filter(p => (sport === "all" || p.sportId === sport) && (tab === "all" || p.authorId === me.id || state.connectedPlayerIds.includes(p.authorId)));
  return <>
    ...
    <div className="post-list">{posts.length ? posts.map(p => <PostCard post={p} key={p.id} />) : <EmptyState title="A resenha ainda vai começar.">Troque o esporte ou acompanhe mais pessoas para encontrar posts por aqui.</EmptyState>}</div>
  </>;
}

...
export function AccessGate({children}:{children:ReactNode}){
 ...
 return <section className="read-message" role="status"><h1>{state==='loading'?'Conferindo seu acesso…':state==='denied'?'Seu acesso está indisponível.':'Não foi possível conferir o acesso.'}</h1><p>{state==='denied'?'Contas suspensas, revogadas ou em exclusão permanecem sem acesso.':'O acesso depende de uma conexão disponível.'}</p>{state!=='loading'&&<div className="read-message-actions"><Link href="/acesso" className={buttonVariants()}>Acesso e minha conta</Link><Button variant="quiet" onClick={()=>location.reload()}>Tentar novamente</Button></div>}</section>
}

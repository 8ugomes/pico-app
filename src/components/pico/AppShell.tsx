...
export function AppShell({ children, environment = 'demo' }: { children: ReactNode; environment?: 'demo' | 'configured' | 'invalid' }) {
  ...
  const connected = environment !== 'demo' && (pathname === '/perfil' || pathname === '/arenas' || pathname.startsWith('/arenas/'));
  ...
  return <div className="social-app">
    ...
    <div className="app-center">
      ...
      {connected ? <div className="demo-banner read-mode-banner"><span className="demo-indicator" />Conta e catálogo <span>· consulta de dados cadastrados</span></div> : <div className="demo-banner"><span className="demo-indicator" />Demonstração <span>· pessoas fictícias, ações nesta sessão</span></div>}

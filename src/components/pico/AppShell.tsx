...
function ConnectedIdentity({ details = false }: { details?: boolean }) {
  ...
  return <><span className="shell-avatar"><RemoteAvatar src={profile?.avatar ?? null} name={profile?.name ?? 'seu perfil'} /></span>{details && <span><strong>{profile?.name || 'Minha conta'}</strong><small>{profile ? `@${profile.username}` : 'Seu perfil no Pico'}</small></span>}</>;
}

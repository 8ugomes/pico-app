...
export function ProfileView({ playerId }: { playerId?: string }) {
  ...
  const playedArenas = mine ? state.arenas.filter(a => state.playedArenaIds.includes(a.id)) : [];
  ...
  return <>
    ...
    {tab === "arenas" && <div className="profile-places"><h2>Arenas acompanhadas</h2><div className="context-links">{arenas.map(a => <Link href={`/arenas/${a.slug}`} key={a.id}>{a.name}</Link>)}</div><h2>Já joguei</h2><p>{mine ? 'Declarações desta demonstração, separadas dos seus jogos privados.' : 'Esta demonstração não mostra declarações desta pessoa.'}</p><div className="context-links">{playedArenas.map(a => <Link href={`/arenas/${a.slug}`} key={a.id}>{a.name} · Demo</Link>)}</div>{!playedArenas.length && <p>Nenhuma arena declarada nesta demonstração.</p>}<h2>Comunidades</h2><div className="context-links">{state.communities.filter(c => c.members.includes(player.id) && (c.visibility === "beta" || c.members.includes(me.id))).map(c => <Link href={`/comunidades/${c.slug}`} key={c.id}>{c.name}</Link>)}</div>{!arenas.length && <EmptyState title="Encontre seu lugar na areia.">Explore as arenas e siga suas comunidades favoritas.</EmptyState>}</div>}
    {mine && <TourLauncher />}

...
export function SearchField({ value, onChange, placeholder, label, tourId, maxLength }: { tourId?: string; maxLength?: number; value: string; onChange: (v: string) => void; placeholder: string; label: string }) {
  return <label data-tour={tourId} className="search-field"><Search size={19} aria-hidden="true" /><span className="sr-only">{label}</span><input type="search" maxLength={maxLength} autoCapitalize="none" spellCheck={false} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} /></label>;
}

...
import { MutationError, exactKeys, mutationFailure, uuid, textField } from '@/lib/supabase/mutations';
...
export async function GET(request: Request) {
  try {
    ...
    const allowed = bucket === 'entity-media' ? await client.rpc('can_read_entity_media',{p_path:path}) : await client.rpc('can_read_media',{p_bucket:bucket,p_path:path});
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const slot = bucket === 'entity-media' ? textField(params.get('slot'), 5, 6) : bucket === 'avatars' ? 'avatar' : 'post';
    ...
    const reserved = bucket === 'entity-media'
      ? await client.rpc('reserve_entity_media', { p_kind: textField(params.get('kind'), 5, 9), p_id: uuid(params.get('id')), p_slot: slot })
    ...
    try {
      ...
      normalized = await decoder.rotate().resize({ width: slot === 'avatar' ? 512 : 1600, height: slot === 'avatar' ? 512 : 1600, fit: 'inside', withoutEnlargement: true }).webp({ quality: 82 }).toBuffer();
    ...
    const ready = bucket === 'entity-media' ? await admin.from('entity_media_assets').update({ ready: true }).eq('path', path).eq('uploaded_by', user.id).eq('deleting', false).select('path').single() : await admin.from('media_assets').update({ ready: true }).eq('path', path).eq('player_id', user.id).select('path').single();
    ...
    if (path && reservedBucket && ownerId) {
      ...
      if (!removed.error) {
        if (reservedBucket === 'entity-media') await admin.from('entity_media_assets').delete().eq('path', path).eq('uploaded_by', ownerId).eq('ready', false);
        else await admin.from('media_assets').delete().eq('path', path).eq('player_id', ownerId).eq('ready', false);
      }
    }

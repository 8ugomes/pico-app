...
export default function SocialLayout({ children }: { children: ReactNode }) {
  ...
  return <DemoProvider><NotificationsProvider demo={environment === 'demo'}>{environment === 'demo' ? shell : <OwnProfileProvider>{shell}</OwnProfileProvider>}</NotificationsProvider></DemoProvider>;
}
